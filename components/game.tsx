...
export function Game() {
  ...
  const feedback = game.phase === "correct" ? "Correct" : game.wrongPick !== null ? "Wrong" : "";
  ...
  return (
    <>
      ...
      <div className="hidden flex-1 flex-col items-center justify-center gap-3 px-6 text-center max-md:portrait:flex">
        ...
        <p className="text-xl font-semibold">Turn your phone sideways</p>
        <p className="text-sm text-dim">Diesis plays in landscape, like a guitar neck.</p>
      </div>
      ...
      <div className="flex flex-1 flex-col max-md:portrait:hidden">
        <header className="flex h-11 items-center justify-between px-4">
          <div className="flex items-center gap-3">
            <Link href="/app" className="flex items-center gap-1 text-sm text-dim hover:text-ink" aria-label="Back to modes">
              <ArrowLeft className="size-4" />
              <span className="hidden sm:inline">Modes</span>
            </Link>
            <h1 className="text-base font-semibold">Name the note</h1>
          </div>
        ...
        <div ref={ref} className="relative mx-3 min-h-0 flex-1">
          ...
          {game.phase === "idle" || game.phase === "loading" ? (
            <div className="absolute inset-0 flex items-center justify-center">
              ...
              >
                <span className="block text-2xl font-semibold">{game.phase === "loading" ? "Loading…" : "Tap to start"}</span>
                <span className="mt-1 block text-sm opacity-80">A note lights up and plays. Name it.</span>
              </button>
            </div>
          ) : null}
        </div>
        ...
        <p className="hidden h-8 items-center justify-center text-xs text-dim pointer-fine:md:flex">
          Keys: C D E F G A B pick a note, Shift for ♯, Space to hear again.
        </p>
