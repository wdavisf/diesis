...
// pitch from E2 (40) to E6 (88), plus the require table Metro needs. Run: npm run samples.
...
const outDir = join(root, 'assets', 'samples', 'nylon');
...
const lines = [
  ...
  'export const SAMPLE_LOW = ' + LOW + ';',
  'export const SAMPLE_HIGH = ' + HIGH + ';',
  'export const NYLON_SAMPLES: Record<number, number> = {',
];
for (let midi = LOW; midi <= HIGH; midi++) {
  const freq = 440 * Math.pow(2, (midi - 69) / 12);
  writeFileSync(join(outDir, `${midi}.wav`), wav(pluck(freq)));
  lines.push(`  ${midi}: require('../../assets/samples/nylon/${midi}.wav'),`);
}
lines.push('};', '');
writeFileSync(join(root, 'src', 'audio', 'samples.generated.ts'), lines.join('\n'));
